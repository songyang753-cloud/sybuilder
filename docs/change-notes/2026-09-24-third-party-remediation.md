...
**This note records implementation and local verification, not stable-release certification.** Two frozen full-suite runs passed as recorded below. Stable quality claims still require an independently executed research-writing trial, production W5 evaluation and live document-platform readback/visual acceptance. Synthetic adapter/CDP fixtures are never production or platform acceptance.
...
Both full runs used separate clean `git archive` extractions of commit `35013e416c025b49ce8a033e11917c1f881cd43b`, running `bash scripts/verify-suite.sh --full` on macOS with Python 3.10.20, Node 22.4.0 and an isolated Chromium runtime. No personal browser profile or live platform account was used.

| Check | Run 1 | Run 2 |
|---|---|---|
| Full verification process | Exit 0, reached final success | Exit 0, reached final success |
| Core selftests | 84 scripts / 1369 cases | 84 scripts / 1369 cases |
| Failures / UNABLE / cache reuse | 0 / 0 / 0 | 0 / 0 / 0 |
| Consistency rules | 46 passed / 0 failed | 46 passed / 0 failed |
| Coding-standard mutations | 45 caught / 0 missed / 0 unable | 45 caught / 0 missed / 0 unable |
| Downstream regression, rendering, golden sample, installation and packaging | Passed | Passed |

The 488 tracked input files excluding generated `.selftest-measured.json` were byte-identical to the commit after both runs. Every per-script measured result also matched across runs. Input-set SHA-256: `b35aa9818fe5e4765c1b8bed7ca8dec3a1272f1e7677e256f8fcef39a8547981` (sorted relative-path to SHA-256 mapping serialized as JSON). The generated measured snapshot is copied unchanged from run 2; totals were not edited by hand. A follow-up commit only records this evidence and documentation, not new runtime logic.

Raw logs remain private because they include local paths. Their SHA-256 values are:

- Run 1: `c5c373395116f6d2e9ec20fc7f898fb77eeec5015014b2d9c8880002d54fda26`.
- Run 2: `43d3b861a729e14a4d29b3404f06017be4483444ab6f38b271a83be5c5a82eed`.

Earlier unsuccessful attempts are retained, not counted as these runs: stale documented counts caused skipped consistency mutations; the first clean committed inventory also exposed the new recovery helper missing from the script count. After those corrections, the full wrapper found three old offline image-review fixture variants missing the newly required metadata. Only the synthetic fixture was migrated; real preflight was not weakened. Full quick verification then passed before restarting both fresh full runs.

## Remaining acceptance work

Public-preview packaging and the current-tree privacy scan (including the external private denylist) passed. This is not an independent research-writing trial, native Feishu/DingTalk compatibility test, production-model evaluation, complete historical privacy certificate or proof of product quality/performance equivalence. Those stable-release blockers remain open in `RELEASE_BLOCKERS.md`. OCR observations remain limited to version/help and local preview; its own runtime preflight and production review need separate evidence. No published history was rewritten and no stable tag is issued by this update.

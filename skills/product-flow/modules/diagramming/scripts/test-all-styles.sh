...
TEST_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sybuilder-styles.XXXXXX")
...
STYLE_NAMES=("Flat Icon" "Dark Terminal" "Blueprint" "Notion Clean" "Glassmorphism" "Warm Editorial" "Minimal Green" "Dark Luxury")
...
for i in "${!STYLES[@]}"; do
    ...
    if [ ! -d "$FIXTURES_DIR" ]; then
        echo "FAIL: Fixtures directory not found: $FIXTURES_DIR"
...
PY
    ...
    if [ "$MATCHED_COUNT" -eq 0 ] && [ -z "$STATIC_FIXTURE_FILES" ]; then
        echo "FAIL: No regression fixtures found for style $STYLE"
        FAILED=$((FAILED + 1)); TOTAL=$((TOTAL + 1))
        continue
    fi
...
PY
        ...
        if python3 "${SKILL_DIR}/scripts/generate-from-template.py" "$TEMPLATE_TYPE" "$SVG_FILE" "$(cat "$FIXTURE")" > /dev/null 2>&1 \
            ...
            if python3 "${SKILL_DIR}/scripts/render-svg.py" "$SVG_FILE" "$PNG_FILE"; then
...
if [ "$FAILED" -eq 0 ] && [ "$TOTAL" -ge 8 ]; then

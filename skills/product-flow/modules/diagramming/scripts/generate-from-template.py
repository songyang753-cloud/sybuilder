...
def render_blueprint_title_block(
    ...
    lines = [
        ...
        f'  <text x="{x + block_width / 2}" y="{y + 64}" text-anchor="middle" font-size="9.5" font-weight="600" fill="{sub_fill}">{center_caption}</text>',
...
def render_rect_node(node: Dict[str, object], style: Dict[str, object], kind: str) -> str:
    ...
    if kind == "bot":
        title_x = x + width / 2
        text_anchor = "middle"
    title_size = max(14, to_float(node.get("title_font_size", 18)))
    lines.append(f'  <text x="{title_x}" y="{title_y}" text-anchor="{text_anchor}" class="node-title" style="font-size:{title_size}px">{title}</text>')
...
def render_node(node: Dict[str, object], style: Dict[str, object]) -> str:
    ...
    if kind == "cylinder":
        ...
        subtitle = normalize_text(node.get("sublabel", ""))
        lines = [
            f'  <path d="M {x} {y + ry} L {x} {y + height - ry} A {rx} {ry} 0 0 0 {x + width} {y + height - ry} L {x + width} {y + ry} Z" fill="{fill}" stroke="{stroke}" stroke-width="{stroke_width}"/>',
...
def build_svg(template_type: str, data: Dict[str, object]) -> str:
    ...
    # Older layout fixtures are supported; explicit endpoints are still checked.
    if data.get('schemaVersion') is not None:
        validate_source(data)
    else:
        ids = [n.get('id', f'node-{i}') for i, n in enumerate(data.get('nodes', []))]
        if len(ids) != len(set(ids)):
            raise ValueError('diagram-nodes: duplicate node id')
        for arrow in data.get('arrows', []):
            if any(k in arrow and arrow[k] not in ids for k in ('source', 'target')):
                raise ValueError('diagram-edge: endpoint does not exist')
            if 'from' in arrow or 'to' in arrow:
                raise ValueError('diagram-edge: use source/target, not from/to')
    ...
    for index, node_data in enumerate(nodes_data):
        ...
        lines.append('<g data-node-id="' + escape(node.node_id, quote=True) +
                     '" data-bounds="' + ','.join(str(x) for x in bounds) + '">')
...
def main() -> None:
    ...
    try:
        ...
        if data.get('schemaVersion') == 1:
            with open(output_path + '.source.json', 'w', encoding='utf-8') as handle:

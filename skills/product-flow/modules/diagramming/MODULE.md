...
> 本模块由 MIT 许可的 `fireworks-tech-graph` 1.0.5 融合而来，原始版权与许可见
> [`LICENSE`](LICENSE)。它是 `product-flow` 的内部库，不是第四个自动触发的 Skill；
> 主流程在 S2、S4、S8 需要真实结构图时按需读取本文件。

模块根目录记为 `DIAGRAM_MODULE_ROOT`。下文为了保留上游命令兼容性仍使用变量名
`SKILL_ROOT`，运行前令 `SKILL_ROOT="$DIAGRAM_MODULE_ROOT"` 即可。

Generate production-quality SVG technical diagrams exported as PNG via `cairosvg`, `rsvg-convert`, or an installed Chrome/Chromium browser.
...
Before reading a reference or running a script, resolve the directory containing this `MODULE.md` as `SKILL_ROOT`. Do not assume the current working directory is the module directory, and do not assume a variable set in one shell call persists into the next. The parent `product-flow` Skill resolves this path as `modules/diagramming` relative to its own root.
...
```bash
SKILL_ROOT="/absolute/path/to/product-flow/modules/diagramming"
...
| 6 | **Warm Editorial** | Warm cream `#f8f6f3` | Anthropic-style diagrams |
| 7 | **Minimal Green** | Pure white `#ffffff` | OpenAI-style diagrams |

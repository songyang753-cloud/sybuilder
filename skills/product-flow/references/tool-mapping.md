...
| S4 图 | `fireworks-tech-graph` → SVG → `rsvg-convert` 转 PNG（**别用 cairosvg，中文豆腐块**） | Mermaid |
| **S2 竞品图** | **同上（同一条链）** —— 竞品侧的核心体验路径/模块图/功能树/页面关系图/关键流程图，⛔ 不另找工具：两套风格会让与 PRD 的「同构对照」失去意义 | Mermaid |
| S5.1 设计系统 | `ui-ux-pro-max --json`（**只取数据不落盘**）→ **选定 `web-design-engineer` 25 个真实配方之一当视觉锚点**（⚠️ 必须在 impeccable 之前，因为配方给的是令牌级精确值）→ `impeccable /init` → 唯一 `DESIGN.md` + `tokens.json`（⭐ 此时是**草案**——B 锁代表切片批准后按实况修订，全量铺开前冻结，见 S7 时序）；组件像素规格并入 `design-system` 的 `component-specs`；中文产品加 `chinese-font-selector` | 手写 `DESIGN.md` 令牌 |
| **S5.1 验证** | 四关：`impeccable /audit` + **impeccable 的** `detect.mjs`（**沉默不可采信**；⚠️ 它不在本 skill 的 `scripts/` 下）+ `token-provenance-gate` + `ai-slop-gate` + 技术可行性 checklist（**含对比度**） | 人工 WCAG checklist |
...
| 协作文档写入 | 飞书 `feishu` CLI / 钉钉 `dws`（⚠️单次≤9000字符须分块；⚠️多列表格会被**静默压列**，4 列可能落地成 2 列） | 输出 Markdown 由人工贴 |
| S6 | `demo-html`（骨架）+ 可选叠 `cc-design`（视觉皮，**顺序不能反**） | 纯手写单文件 HTML，仍须满足零外链与自测门禁 |
| S7 | `figma` MCP `use_figma` 直写（账号与前置见下） | 输出标注齐全的设计规格由设计师执行 |
| S8 | 飞书 CLI + `s8-solution-plan.md` + `s8-solution-gate.py`；技术/算法/测试三专业评审；引用 `coding-standards`/`four-node-review` 锚定版本 | 没有飞书时交受控 md，最高 `review-ready`，⛔ 不冒充飞书原生交付 |

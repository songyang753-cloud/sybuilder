#!/usr/bin/env python3
"""dingtalk supplied-artifact delivery checks. 0=PASS, 1=FAIL, 2=UNABLE.
...
def _media_entities(value):
    entities = set()
    def visit(item):
        if isinstance(item, dict):
            kind = str(item.get('type') or item.get('blockType') or item.get('resourceType') or '').lower()
            locator = item.get('mediaId') or item.get('resourceId') or item.get('fileId')
            if kind in {'image', 'picture'} and isinstance(locator, str) and locator.strip():
                entities.add(locator)
...
def check(source_path, readback_path, manifest_path=None, markdown_readback=None):
    return shared_check(source_path, readback_path, manifest_path, markdown_readback, 'dingtalk')
...
def self_test():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        native = json.dumps({'content': '# Report\n\n## Feature\n\nUsers may cancel.', 'blocks': [{'type': 'image', 'mediaId': 'm1'}]})
...
def main():
    ...
    if a.receipt:
        Path(a.receipt).write_text(json.dumps({'gate': 'dingtalk-delivery-gate',
